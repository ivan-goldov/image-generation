...
def train(data, G_AB, G_BA, D_A, D_B, 
    ...
    lr_scheduler_G = torch.optim.lr_scheduler.LambdaLR(optimizer_G, 
                                                       lr_lambda=LambdaLR(n_epochs, 0, n_epochs/2).step)
    ...
    with tqdm(desc="epoch", total=n_epochs) as pbar_outer:
        for epoch in range(shift, n_epochs+shift):
            lg = 0
            la = 0
            lb = 0
            k = 1
            ...
            losses_gan.append(lg / k)
            losses_a.append(la / k)
            losses_b.append(lb / k)

            if epoch % 20 == 19:
                if save_losses:
                    with open('/losses/lossgan_epoch{}'.format(epoch), 'wb') as f:
                        pickle.dump(losses_gan, f)
                    with open('/losses/losses_a_epoch{}'.format(epoch), 'wb') as f:
                        pickle.dump(losses_a, f)
                    with open('/losses/losses_b_epoch{}'.format(epoch), 'wb') as f:
                        pickle.dump(losses_b, f)

                if save_models:
                    torch.save(G_AB.state_dict(), '/saved-models/G_AB_epoch{}'.format(epoch))
                    torch.save(G_BA.state_dict(), '/saved-models/G_BA_epoch{}'.format(epoch))
                    torch.save(D_A.state_dict(), '/saved-models/D_A{}'.format(epoch))
                    torch.save(D_B.state_dict(), '/saved-models/D_B{}'.format(epoch))
                if real_A.shape[1] != 1:
                    with torch.no_grad():
                        plot(real_A, G_AB(real_A), G_BA(real_A))
